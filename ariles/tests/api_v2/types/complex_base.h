...
#    endif
            ...
            ariles2::apply<ariles2::Finalize>(*impl);
        }
#endif
    };
    ...
    void compare(const t_Configurable_out &configurable_out, const t_Configurable_in &configurable_in)
    {
        ...
        for (std::size_t i = 0; i < configurable_out.std_nested_vector_.size(); ++i)
        {
            ...
            {
                BOOST_CHECK_CLOSE(
                        configurable_out.std_nested_vector_[i][j],
...
#    ifdef ARILES_ADAPTER_EIGEN
        ...
        BOOST_CHECK(
                configurable_out.matrix_complex_double_.isApprox(configurable_in.matrix_complex_double_, g_tolerance));
        ...
        {
            BOOST_CHECK_EQUAL(
                    configurable_out.std_nested_vector_evector_[i].size(),
                    configurable_in.std_nested_vector_evector_[i].size());
