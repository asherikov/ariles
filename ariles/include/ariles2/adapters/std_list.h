...
namespace ariles2
{
    ...
    bool isMissing(const std::list<t_Args...> &entry)
    {
        return (entry.empty());
    }
}  // namespace ariles2
...
namespace ariles2
{
    namespace read
    {
        ...
        void apply_read(t_Visitor &visitor, std::list<t_Args...> &entry, const typename t_Visitor::Parameters &param)
        {
            ...
            {
                typename std::list<t_Args...>::value_type value;
...
namespace ariles2
{
    namespace write
    {
                ...
                const typename t_Visitor::Parameters &param)
        {
            ...
            for (const typename std::list<t_Args...>::value_type &value : entry)
            {
...
namespace ariles2
{
    namespace compare
    {
        ...
        void apply_compare(
                t_Visitor &visitor,
                const std::list<t_Args...> &left,
                const std::list<t_Args...> &right,
                const typename t_Visitor::Parameters &param)
        {
            CPPUT_TRACE_FUNCTION;
            ...
            typename std::list<t_Args...>::const_iterator left_it = left.begin();
            typename std::list<t_Args...>::const_iterator right_it = right.begin();
...
namespace ariles2
{
    namespace process
    {
        ...
        {
            CPPUT_TRACE_FUNCTION;
            for (typename std::list<t_Args...>::reference value : entry)
            {
...
namespace ariles2
{
    namespace copyfrom
    {
                ...
                const typename t_Visitor::Parameters &param)
        {
            ...
            left.clear();
            for (const typename std::list<t_RightArgs...>::value_type &right_value : right)
            {
                typename std::list<t_LeftArgs...>::value_type left_value;
    ...
    namespace copyto
    {
        ...
        void apply_copyto(
                t_Visitor &visitor,
                const std::list<t_LeftArgs...> &left,
                std::list<t_RightArgs...> &right,
                const typename t_Visitor::Parameters &param)
        {
            CPPUT_TRACE_FUNCTION;

            right.clear();
            for (const typename std::list<t_LeftArgs...>::value_type &left_value : left)
            {
                typename std::list<t_RightArgs...>::value_type right_value;
